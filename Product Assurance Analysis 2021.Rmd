---
title: "Product Assurance Analysis (2020)"
...
## Product Assurance Analysis: 2020
...
Vessel_table <- read_excel("2020_Vessel.xlsx")
...
ggplot(Sink_related_under20_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "Number of Vessel Incidents", fill="test", x = "Incident Cause", title = "Number of Sink/Swamp/Capsize Related Incidents by Causes Listed", subtitle = "For vessels under 20 feet") + 
...
ggplot(df_VesselMachineryFailure_Sinkunder20, aes(x = VesselMachineryFailure, y = counts)) +
  geom_bar(aes(reorder(VesselMachineryFailure, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "Number of Vessel Incidents", fill="test", x = "Machinery Failure (Type)", title = "Number of Sink/Swamp/Capsize Related Incidents by\nMachinery Failure Reported", subtitle = "For vessels under 20 feet") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) +
...
ggplot(Sink_related_20to26_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "Number of Vessel Incidents", fill="test", x = "Incident Cause", title = "Number of Sink/Swamp/Capsize Related Incidents by Causes Listed", subtitle = "For vessels 20'-26' in length") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 200)) 
```
...
ggplot(All_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "(n)", fill="test", x = "Incident Cause", title = "Total Incidents by Causes Listed", subtitle = "(n > 100)") + 
  theme(axis.text.x = element_text(angle = 66, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 1500)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))

# Now, lets do it in percents with 12,484 cases that we know (with no NAs)

All_CauseTally2 <- All_CauseTally %>%
  group_by(VesselCause1) %>%
  filter(VesselCause1!="na") %>%
  summarise(counts = n())  %>%
  mutate("percent"=counts/12484*100) %>%
  filter(percent>=5)
...
ggplot(All_CauseTally2, aes(x = VesselCause1, y= percent )) +
  ...
  geom_text(aes(label = round(percent,2)), vjust=-0.5)  + 
    labs(y = "%", fill="test", x = "Incident Cause", title = "Total Incidents by Causes Listed", subtitle = "(5% or higher)") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 100)) 
```
...
ggplot(Pontoon_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "(n)", fill="test", x = "Incident Cause", title = "Total Pontoon Incidents by Causes Listed", subtitle="(n > 10)") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 175)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))

# Now, lets do it in percents with 979 cases that we know (with no NAs)
Pontoon_CauseTally2 <- Pontoon_CauseTally %>%
  group_by(VesselCause1) %>%
  filter(VesselCause1!="na") %>%
  summarise(counts = n())  %>%
  mutate("percent"=counts/979*100) %>%
  filter(percent>=5)
...
ggplot(Pontoon_EventTally1, aes(x = VesselEvent1, y= counts )) +
  ...
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "(n)", fill="test", x = "Incident Event", title = "Total Pontoon Incidents by Events Listed", subtitle="(n > 10)") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 200)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))

# Now, lets do it in percents with 871 cases that we know (with no NAs)
Pontoon_EventTally2 <- Pontoon_EventTally %>%
  group_by(VesselEvent1) %>%
  filter(VesselEvent1!="na") %>%
  summarise(counts = n())  %>%
  mutate("percent"=counts/871*100) %>%
  filter(percent>=5)
...
ggplot(Pontoon_EventTally2, aes(x = VesselEvent1, y= percent )) +
  ...
  geom_text(aes(label = round(percent,2)), vjust=-0.5)  + 
    labs(y = "%", fill="test", x = "Incident Event", title = "Percentage of Pontoon Incidents by Events Listed", subtitle="(5% or higher)") + 
...
ggplot(PWC_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "(n)", fill="test", x = "Incident Cause", title = "PWC Incidents by Causes Listed", subtitle="(n > 10)") + 
  theme(axis.text.x = element_text(angle = 66, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 600)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))

# Now, lets do it in percents with 2,874 cases that we know (with no NAs)
PWC_CauseTally2 <- PWC_CauseTally %>%
  group_by(VesselCause1) %>%
  filter(VesselCause1!="na") %>%
  summarise(counts = n())  %>%
  mutate("percent"=counts/2874*100) %>%
  filter(percent>=5)
...
ggplot(PWC_CauseTally2, aes(x = VesselCause1, y= percent )) +
  geom_bar(aes(reorder(VesselCause1, percent), percent), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = round(percent,2)), vjust=-0.5)  + 
    labs(y = "%", fill="test", x = "Incident Cause", title = "Percentage of PWC Incident Causes Listed", subtitle="(5% or higher)") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 100)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))
```
...
ggplot(PWC_EventTally1, aes(x = VesselEvent1, y= counts )) +
  geom_bar(aes(reorder(VesselEvent1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "(n)", fill="test", x = "Incident Event", title = "PWC Incidents by Events Listed", subtitle="(n > 50)") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 1000)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))

# Now, lets do it in percents with 2,625 cases that we know (with no NAs)
PWC_EventTally2 <- PWC_EventTally %>%
  group_by(VesselEvent1) %>%
  filter(VesselEvent1!="na") %>%
  summarise(counts = n())  %>%
  mutate("percent"=counts/2625*100) %>%
  filter(percent>=5)
...
ggplot(PWC_EventTally2, aes(x = VesselEvent1, y= percent )) +
  geom_bar(aes(reorder(VesselEvent1, percent), percent), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = round(percent,2)), vjust=-0.5)  + 
    labs(y = "%", fill="test", x = "Incident Event", title = "Percentage of PWC Incidents by Events Listed", subtitle="(5% or higher)") + 
  theme(axis.text.x = element_text(angle = 40, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 100)) +
theme(axis.title.y = element_text(angle = 0, vjust = .7))
```
...
Now, specifically focusing on equipment failures.
There is one, listed as an "auxiliary" failure
```{r}
...
## VI. Ad Hoc Analysis for Jeff
```{r}
Outboard_Under26 <- Vessel_table %>%
  filter(Length<=26,EngineType=="Outboard")

NonOutboard_Under26 <- Vessel_table %>%
  filter(Length<=26, EngineType!="Outboard")
```

```{r echo=FALSE}
Fuel_related1a <- Outboard_Under26 %>%
  filter(VesselEvent1=="Fire/explosion (fuel)") 

Fuel_related2a <- Outboard_Under26 %>%
  filter(VesselEvent2=="Fire/explosion (fuel)")

Fuel_related3a <- Outboard_Under26 %>%
  filter(VesselEvent3=="Fire/explosion (fuel)")

Fuel_related_Outboard <- rbind(Fuel_related1a, Fuel_related2a, Fuel_related3a)

# This is another way to accomplish the above code chunk. Count how many instances each VesselCause comes up
df_VesselCause1_fuelOutboard<- Fuel_related_Outboard %>%
  group_by(VesselCause1) %>%
  summarise(counts = n()) 

df_VesselCause2_fuelOutboard<- Fuel_related_Outboard %>%
  group_by(VesselCause2) %>%
  summarise(counts = n()) 

df_VesselCause3_fuelOutboard<- Fuel_related_Outboard %>%
  group_by(VesselCause3) %>%
  summarise(counts = n()) 

# If you summarize the values for each of the three dataframes above, you can get to how many total causes are mentioned for either cause1, cause2, or cause3
df_VesselCause1_fuelOutboard
df_VesselCause2_fuelOutboard
df_VesselCause3_fuelOutboard
```
### A. Fuel Problems
```{r echo=FALSE}
df_VesselCause1_fuelOutboard <- Fuel_related_Outboard %>%
...
Fuel_related_20to26_CauseTally <- rbind(df_VesselCause1_fuelOutboard, df_VesselCause2_fuelOutboard, df_VesselCause3_fuelOutboard)
...
ggplot(Fuel_related_20to26_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "Number of Vessel Incidents", fill="test", x = "Incident Cause", title = "Number of Fire/Explosion (Fuel) Incidents by Causes Listed", subtitle = "For Outboard vessels <=26' in length") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
...
### B. Electrical Problems 
```{r echo=FALSE}
Electrical_under20a <- NonOutboard_Under26 %>%
  filter(VesselMachineryFailure!= "na", VesselMachineryFailure=="Electrical") %>%
  select(BARDID, RegistrationNumber, HIN, VesselMachineryFailure, Manufacturer, VesselCause1, VesselCause2, VesselCause3) 
...
## Non-Outboard Analysis
```{r}
Fuel_related1b <- NonOutboard_Under26 %>%
  filter(VesselEvent1=="Fire/explosion (fuel)") 

Fuel_related2b <- NonOutboard_Under26 %>%
  filter(VesselEvent2=="Fire/explosion (fuel)")

Fuel_related3b <- NonOutboard_Under26 %>%
  filter(VesselEvent3=="Fire/explosion (fuel)")

Fuel_related_NonOutboard <- rbind(Fuel_related1b, Fuel_related2b, Fuel_related3b)
```

```{r echo=FALSE}
# This is another way to accomplish the above code chunk. Count how many instances each VesselCause comes up
df_VesselCause1_fuelNonOutboard<- Fuel_related_NonOutboard %>%
  group_by(VesselCause1) %>%
  summarise(counts = n()) 

df_VesselCause2_fuelNonOutboard<- Fuel_related_NonOutboard %>%
  group_by(VesselCause2) %>%
  summarise(counts = n()) 

df_VesselCause3_fuelNonOutboard<- Fuel_related_NonOutboard %>%
  group_by(VesselCause3) %>%
  summarise(counts = n()) 

# If you summarize the values for each of the three dataframes above, you can get to how many total causes are mentioned for either cause1, cause2, or cause3
df_VesselCause1_fuelNonOutboard
df_VesselCause2_fuelNonOutboard
df_VesselCause3_fuelNonOutboard
```
...
Fuel_related_NonOutboard_CauseTally <- rbind(df_VesselCause1_fuelNonOutboard, df_VesselCause2_fuelNonOutboard, df_VesselCause3_fuelNonOutboard)

Fuel_related_NonOutboard_CauseTally1 <- Fuel_related_NonOutboard_CauseTally %>%
  group_by(VesselCause1) %>%
  filter(VesselCause1!="na") %>%
  summarise(counts = n())

# Now, lets graph it up! Finally..
ggplot(Fuel_related_NonOutboard_CauseTally1, aes(x = VesselCause1, y= counts )) +
  geom_bar(aes(reorder(VesselCause1, counts), counts), fill = "dodgerblue", stat = "identity") +
  geom_text(aes(label = counts), vjust = -0.3) +
    labs(y = "Number of Vessel Incidents", fill="test", x = "Incident Cause", title = "Number of Fire/Explosion (Fuel) Incidents by Causes Listed", subtitle = "For Non-outboard vessels<=26' in length") + 
  theme(axis.text.x = element_text(angle = 50, hjust = 1)) + 
  scale_y_continuous(limits = c(0, 100)) 
```
## VII. Product Assurance Overview Graph
```{r}
library(tidyverse)
library(readxl)
library(dplyr)
...
Primarydata <- read_excel("Temp_doc.xlsx")

DF <- data.frame (group = c("Deaths", "Injuries", "Property damage only"),
                  `Carbon monoxide exposure`= c(5, 40, 0),
                  `Fire/explosion (fuel)`=c(1, 19, 15),
                  `Fire/explosion (non-fuel)`=c(0,2,22))

DFtall <- DF %>% gather(key= Incident, value=Value, Carbon.monoxide.exposure:Fire.explosion..non.fuel.) 
DFtall <- DFtall %>%
  filter(Value!=0)

P1 <- ggplot(DFtall, aes(Incident, Value, fill = group)) + geom_col(position = "dodge") 

P2 <- P1 + labs(fill = "") +  scale_x_discrete(limits = c("Carbon.monoxide.exposure", "Fire.explosion..non.fuel.", "Fire.explosion..fuel."),
                   labels = c("Carbon monoxide exposure", "Fire/explosion (non-fuel)", 
                              "Fire/explosion (fuel)")) + ylab("") + xlab("Incident Type") +
  scale_y_continuous(limits = c(0,45))  + labs(title = "Product Assurance Casualty Overview (2020)") +
  geom_text(aes(label = Value), position = position_dodge(0.9), vjust=-0.5)  +
  scale_colour_manual(values=c("red", "dodgerblue", "grey"))

P2 + scale_fill_manual(values=c("red", "dodgerblue", "grey"))
```
